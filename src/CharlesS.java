...
public class CharlesS {
    ...
    public static class CharlesSchuck implements Player{
        ...
        public Card playFirstCard(ArrayList<Card> hand, ArrayList<Card> playedCards, Card trump, int tricks1, int tricks2) {
            ...
            Card result = resultN.getPlay();

            if(System.currentTimeMillis() > time + allTime){
                overTimes++;
                System.out.println("CS P1 RESP IN: " + (System.currentTimeMillis() - time) + " ms -- total overtimes: " + overTimes);
            }
            //System.out.println("CE P1 RESP IN: " + (System.currentTimeMillis() - time) + " ms -- total overtimes: " + overTimes);
            return result;
        }
        ...
        public Card playSecondCard(ArrayList<Card> hand, ArrayList<Card> playedCards, Card trump, int tricks1, int tricks2) {
            ...
            if(System.currentTimeMillis() > time + allTime){
                overTimes++;
                System.out.println("CS P2 RESP IN: " + (System.currentTimeMillis() - time) + " ms -- total overtimes: " + overTimes);
            }
            //System.out.println("CE P2 RESP IN: " + (System.currentTimeMillis() - time) + " ms -- total overtimes: " + overTimes);
            return result;
        }

        public Node alphaBetaParkingLot(Node state, boolean player1, int alpha, int beta, double time){
            if(state.hand.isEmpty() || System.currentTimeMillis() > time + allTime){ //the exit scenario.
                return state;
            }
            int bestEvaluation;
    ...
    static class Node{
        ...
        Node(ArrayList<Card> hand, ArrayList<Card> playedCards, Card trump, int tricks1, int tricks2){ //incoming constructor
            this.hand = hand;
        ...
        public void generateRemainingCards(){
            for(int i = 0; i < 10; i++) { //let's just ignore some seemingly trivially small cards.
                for (int j = 0; j < 3; j++) {
        ...
        public void assume(){
            ...
            //if(playedCards.size() % 2 == 1){ //if we are player 2. --
            // I don't know if there's any assumptions that I can make about what player1 is about to play... I mean.. they can play anything they want.
            //} I can't even reasonably assume they'll play the 'best card' for the situation, because the opponent's ai might suck.

            //If my opponent can play anything... then hell... I can't say anything about what they're about to play.
        }
